...
import { FaUser, FaEnvelope, FaPhone, FaMapMarkerAlt, FaIdCard, FaBirthdayCake, FaSave, FaStar, FaCamera } from 'react-icons/fa';
...
const ProfilePage = () => {
  ...
  const handleAvatarChange = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      const key = `avatar_${user?.email}`;
      localStorage.setItem(key, base64);

      // Cập nhật user context nếu có hàm updateProfile
          ...
          <Card className="hotel-card border-0 mb-4 text-center">
            <Card.Body className="p-4 p-md-5">
              <div className="position-relative d-inline-block mb-3">
                <img 
                  src={avatarPreview || user.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=0D1B2A&color=fff`} 
                  alt={user.name} 
              ...
              {avatarSuccess && (
                <div className="alert alert-success py-1 px-3 mb-2" style={{ fontSize: '0.82rem' }}>
              ...
              <Button variant="outline-primary" className="w-100" onClick={() => fileInputRef.current?.click()}>
                <FaCamera className="me-2" /> Đổi ảnh đại diện
              </Button>
            </Card.Body>
          </Card>
        </Col>
